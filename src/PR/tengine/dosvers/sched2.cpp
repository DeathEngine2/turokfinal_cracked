...
void __scHandleRetrace(OSSched *sc)
{
...
#ifdef SC_TIMING
	rmonPrintf("%.1f ms: Retrace\n", ((float)CYCLES_TO_NSEC(osGetTime() - startTime))/1000000);
#endif
	...
	if (n_waiting)
	{
		...
		// ((c_field + 2) >= pFrameData->m_nPredictFields)
		//if (c_field >= pFrameData->m_nPredictFields)
		{
#ifdef SC_FIC
...
/*
	...
	{
		c_field = 0;
		n_pending -= prevPending;
	}
*/
/*
	if (n_created_frames && allow_task_sends)
	{
		n_created_frames--;

		p_created_frames[c_send_frame]->m_RSPComplete = FALSE;
		CEngineApp__SendGraphicsTask(GetApp(), p_created_frames[c_send_frame]);
#ifdef DB_LOOP
		rmonPrintf("%x: sent\n", p_created_frames[c_send_frame]);
#endif
...
*/
	...
	if (n_waiting)
	{
		pFrameData = p_waiting_frames[c_finished];
		if ((c_field + 1) >= pFrameData->m_nPredictFields)
		//if ((c_field + prevPending) >= pFrameData->m_nPredictFields)
		{
			p_pending_fb = pFrameData->m_pFrameBuffer;
			osViSwapBuffer(pFrameData->m_pFrameBuffer);
			n_waiting--;
	...
	if (n_waiting)
	{
		pFrameData = p_waiting_frames[c_finished];
		//if ((c_field + 1) >= pFrameData->m_nPredictFields)
		if ((c_field + 1 + prevPending) >= pFrameData->m_nPredictFields)
		//if (c_field >= pFrameData->m_nPredictFields)
		{
			//rmonPrintf("retrace- actual:%d  predict:%d\n", c_field+1, pFrameData->m_nPredictFields);
...
void __scHandleRDP(OSSched *sc)
{
	...
	// TEMP
	//osSyncPrintf("__scHandleRDP()\n");

	 //ASSERT(!sc->curRDPTask || (sc->curRDPTask != next_rdp_task));
	...
	if (yielded)
	{
		rmonPrintf("rdp yielded\n");
		sc->curRDPTask = 0;
	}
	else
	{
		sc->curRDPTask = next_rdp_task;
		next_rdp_task = 0;
	}
	...
	if (t->list.t.type == M_GFXTASK)
	{
		...
		pApp = GetApp();
		if (t == &pApp->m_FrameData[0].m_Task)
			pFrameData = &pApp->m_FrameData[0];
		else if (t == &pApp->m_FrameData[1].m_Task)
			pFrameData = &pApp->m_FrameData[1];
		else if (t == &pApp->m_FrameData[2].m_Task)
		...
		// RDP complete for graphics task
		osSendMesg(t->msgQ, t->msg, OS_MESG_BLOCK);
	}
  __scTaskComplete(sc, t);
	}
...
#endif
#ifdef SC_TIMING
		rmonPrintf("%.1f ms: Task Complete\n", ((float)CYCLES_TO_NSEC(osGetTime() - startTime))/1000000);
#endif
    ...
    ASSERT(sc->curRSPTask == 0);
	 ...
	 if (dp)
	 {
		 ...
		 else
		 {
			...
			if (sc->curRDPTask)
			{
				ASSERT(dp != next_rdp_task);
				
				if ((sc->curRDPTask != dp) && (next_rdp_task != dp))
				{
					//ASSERT(dp != next_rdp_task);
					//ASSERT(!next_rdp_task);
...
// chedules the tasks to be run on the RCP
...
#endif    
#ifdef SC_TIMING
	rmonPrintf("%.1f ms: Task Submitted\n", ((float)CYCLES_TO_NSEC(osGetTime() - startTime))/1000000);
#endif
...
///*
//							 if (avail & OS_SC_SP) {	// if SP is available
							 if ((avail & OS_SC_SP) && !next_rdp_task) {	// if SP is available
#ifdef SC_LOGGING
                      osLogEvent(l, 519, 0);
#endif                      
                          *sp = gfx;
