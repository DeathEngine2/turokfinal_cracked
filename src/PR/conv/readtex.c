 ...
 * June 21st, 1994
 *
 * $Id: readtex.c,v 1.47 1996/04/02 21:41:07 hsa Exp $
 */
 ...
 */
void 
RGBtoUYVY(unsigned short *rp,unsigned short *gp,unsigned short *bp,unsigned char *ap,int xsize);
...
{
	switch (fmt) {
		case RGBA:	return "G_IM_FMT_RGBA";
		case YUV:	return "G_IM_FMT_YUV";
		case CI:	return "G_IM_FMT_CI";
		case IA:	return "G_IM_FMT_IA";
		case I:		return "G_IM_FMT_I";
		case A:		return "G_IM_FMT_A";
		case MASK:	return "G_IM_FMT_MASK";
	}
...
{
	switch (siz) {
		case 4:		return "G_IM_SIZ_4b";
		case 8:		return "G_IM_SIZ_8b";
		case 16:	return "G_IM_SIZ_16b";
		case 32:	return "G_IM_SIZ_32b";
		default:	return "G_IM_SIZ_OTHER";
	}
...
printCheader(int siz, IMAGE *im, int fmt, struct texture *tex)
{
	...
	if (siz == 4) {
	    printf (" *   gsDPLoadTextureBlock_4b (%s, %s, %d, %d, 0,\n",
		    tex->name, fmtstr (fmt), im->xsize, im->ysize);
	} else {
	    printf (" *   gsDPLoadTextureBlock (%s, %s, %s, %d, %d, 0,\n",
		    tex->name, fmtstr (fmt), sizstr (siz), im->xsize,
		    im->ysize);
	}
...
int PrintCIColorMap(int output, int flags)
{
  ...
  for (i=0; i< 256; i++) {
	...
	else /* replicate each entry 4 times */
	{
	  ...
	  NumberOfColorsSeen++;
	  if (NumberOfColorsSeen % 8 == 0) printf("\n");	  
  };

  return( 256 );
}

int CIColorIndexValue(IMAGE *im, int siz, int fmt, int i, 
		      ushort red[MAX_WIDTH], ushort green[MAX_WIDTH], ushort blue[MAX_WIDTH], 
		      ushort alpha[MAX_WIDTH], ushort intensity[MAX_WIDTH], int output,
		      int x)
{
...
printCIdata(IMAGE *im, int siz, int fmt, int i, 
	    ushort red[MAX_WIDTH], ushort green[MAX_WIDTH], ushort blue[MAX_WIDTH], ushort alpha[MAX_WIDTH], ushort intensity[MAX_WIDTH], int output, int shuffle_mask, int flags)
{
  ...
  for (x = 0; x < im->xsize; x++)
    {
      ...
      if (siz == 8)
	{
	  BestMatch1 = CIColorIndexValue(im, siz, fmt, i, red, green, blue, alpha, intensity, output, xx);
	...
	{
	  BestMatch1 = CIColorIndexValue(im, siz, fmt, i, red, green, blue, alpha, intensity, output, xx);
	  BestMatch2 = CIColorIndexValue(im, siz, fmt, i, red, green, blue, alpha, intensity, output, xx+1);
	  if (!(flags & SKIP_RAW_FLAG)) {
	    if(output == C)
 ...
 *
 */
void RGBtoUYVY(rp,gp,bp,ap,xsize) 
...
int
printdata(IMAGE *im, int siz, int fmt, int i, 
	   ushort red[MAX_WIDTH], ushort green[MAX_WIDTH], ushort blue[MAX_WIDTH], ushort alpha[MAX_WIDTH], ushort intensity[MAX_WIDTH], int output, int flags, int shuffle_mask)
{
  int xx, x, data;
  ...
  /* Some of these are going to break for odd sizes, need to be fixed */
	...
	if (siz == 4 && fmt == I) {
		...
		for (x = 0; x < im->xsize; x++) {
		  ...
		  r = red[xx]   + (r & 0x7); if( r > 255 ) r = 255;
		  g = green[xx] + (g & 0x7); if( g > 255 ) g = 255;
		  b = blue[xx]  + (b & 0x7); if( b > 255 ) b = 255;
		...
		RGBtoUYVY( red, green, blue, uyvy, im->xsize );
		...
		for (x = 0; x < im->xsize; x++) {
		  ...
		  xx = x ^ shuffle_mask;
		        if (!(flags & SKIP_RAW_FLAG)) {
			  printf ("0x%.2x, 0x%.2x, 0x%.2x, 0x%.2x, ", red[xx], green[xx], 
				blue[xx], alpha[xx]);
		        }
			  if (output == MIPMAP) {
			      *savedp8++ = red[xx];
		...
		if (output == MIPMAP) {
		  ...
		  printCIdata(im, siz, fmt, i, red, green, blue, alpha, intensity, output, shuffle_mask,flags);
        }
...
struct texelSizeParams sizeParams[4] =
					{ 16, 3, 1, 0,
					   8, 2, 2, 1,
					   4, 1, 4, 2,
					   2, 0, 8, 3
...
void stuffDisplayList(Gfx **glistp, Image *im, char *tbuf, 
...
void  kernel(int i, int r1, int g1, int b1, int a1, float *r2, float *g2, 
		float *b2, float *a2);
...
int guLoadTextureBlockMipMapPrint(char *name, unsigned char *tbuf, Image *im, 
 unsigned char pal, unsigned char cms, unsigned char cmt, unsigned char masks,
 unsigned char maskt, unsigned char shifts, unsigned char shiftt, int flags,
 unsigned char wcsf, unsigned char wctf)
{
			...
			switch (im->fmt)
			{
			   ...
			   case (G_IM_FMT_RGBA):
				...
				if (MipMapFile.levels >= level) {
				    texel = ((u16 *) MipMapFile.image[level-1])
								[s/2 + (t/2)*mipmap[level].w];
				}
			   ...
			   case (G_IM_FMT_CI):	
			  	...
			  	if (im->siz == G_IM_SIZ_4b)
				{
					...
					if (MipMapFile.levels >= level) {
				            texel = ((u8 *) MipMapFile.image[level-1])
								[s/4 + (t/2)*mipmap[level].w];
 ...
 * stuffDisplayList(glistp, im, tbuf, pal, cms, cmt, masks, maskt, shifts, shiftt);
 */
{ /* stuff display list */
   int tile;   
   int x,y;
   ...
   if( flags & XTRA_FLAG )
       printf("static Gfx %s_dummy_aligner2[] = { gsSPEndDisplayList(), gsSPEndDisplayList(), gsSPEndDisplayList(), gsSPEndDisplayList() };\n\n", name);
   else
   ...
   if (im->siz == G_IM_SIZ_4b) {
   	...
   	printf("\tgsDPSetTile( %d, %d, %d, %d, %s, %d, %d, %d, %d, %d, %d, %d),\n",
	 	im->fmt, G_IM_SIZ_8b, NA, 0, "G_TX_LOADTILE", NA,
         	NA, NA, NA, NA, NA, NA);
   	ndls++;
   	...
   	/* set tile no 7. for loading texture */
   	printf("\tgsDPSetTile( %d, %d, %d, %d, %s, %d, %d, %d, %d, %d, %d, %d),\n",
	 	im->fmt, im->siz, NA, 0, "G_TX_LOADTILE", NA,
         	NA, NA, NA, NA, NA, NA);
   	ndls++;
   ...
   for (tile = 0; tile <= level; tile ++) {
      if (maskt == 255 ) maskt = 0; /* unsigned char */
      if (masks == 255 ) masks = 0;
      printf("\tgsDPSetTile(%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d),\n",
	im->fmt, im->siz, (mipmap[tile].w*txlsize >> 4),
	(mipmap[tile].addr >>  3), tile, pal, cmt, (maskt--), tile, cms,
	(masks--), tile);
      ndls++;
      if (flags & HALF_SHIFT) 
        printf("\tgsDPSetTileSize( %d,  %d, %d, %d << %s, %d << %s),\n",
	  tile, 2, 2,(mipmap[tile].s-1), "G_TEXTURE_IMAGE_FRAC",
	  (mipmap[tile].t-1) , "G_TEXTURE_IMAGE_FRAC");
      else
        printf("\tgsDPSetTileSize( %d,  %d, %d, %d << %s, %d << %s),\n",
	  tile, 0, 0,(mipmap[tile].s-1), "G_TEXTURE_IMAGE_FRAC",
	  (mipmap[tile].t-1) , "G_TEXTURE_IMAGE_FRAC");
      ndls++;
...
void
mipmap_it(IMAGE *im, int siz, int fmt, char *name, int flags)
{
    static Image img;
    ...
    switch( fmt ) {
	case I:   imp->fmt  = G_IM_FMT_I; break;
	case A:   imp->fmt  = G_IM_FMT_I; break;
	case IA:  imp->fmt  = G_IM_FMT_IA; break;
	case CI:  imp->fmt  = G_IM_FMT_CI; break;
	case YUV: imp->fmt  = G_IM_FMT_YUV; break;
	case RGBA:imp->fmt  = G_IM_FMT_RGBA; break;
	default:
    ...
    switch( siz ) {
	case 4:   imp->siz  = G_IM_SIZ_4b;    break;
	case 8:   imp->siz  = G_IM_SIZ_8b;    break;
	case 16:  imp->siz  = G_IM_SIZ_16b;   break;
	case 32:  imp->siz  = G_IM_SIZ_32b;   break;
	default:  imp->siz  = G_IM_SIZ_32b; break;
    };
    ...
    while ((im->xsize >> masks) > 1) masks++; 
    while ((im->ysize >> maskt) > 1) maskt++; 

    /* generate mipmaps, load data and set tiles */
    guLoadTextureBlockMipMapPrint( name, tbuf, imp, 0, G_TX_WRAP | G_TX_NOMIRROR,
        G_TX_WRAP | G_TX_NOMIRROR, masks, maskt, G_TX_NOLOD, G_TX_NOLOD, flags, 0, 0);
...
int readtex (char *fn, struct texture *tex, int fmt, int siz, int makestatic,
	int lr, int lg, int lb, int hr, int hg, int hb, int output, int flags, int shuffle_mask)
{
	IMAGE	*im;
	...
	if (fmt == CI) 
	  {
	    if (flags & MM_SUPPLIED_FLAG) {
	      printf("supplied mip maps not currently supported with Color Index (CI) textures\n");
	      return 0;
	    }
	    if (siz == 4)
	      QuantLevels = 16;
	    ...
	    if (!exec_cmd("/usr/sbin/cscale %s %s.fooa 2040 2040 2040 -d",
			fn, TEMPFILENAME))
		return 0;
	    if (!exec_cmd("/usr/sbin/cscale %s.fooa %s.foob 2040 2040 2040",
			TEMPFILENAME, TEMPFILENAME))
		return 0;
	    if (!exec_cmd("/usr/sbin/toppm %s.foob %s.ppm",
			  TEMPFILENAME, TEMPFILENAME))
		return 0;
	    if (!exec_cmd("$ROOT/usr/sbin/ppmquant %d %s.ppm > %s",
			  QuantLevels, TEMPFILENAME, TEMPFILENAME))
		return 0;
	    if (!exec_cmd("/usr/sbin/fromppm %s %s.rgb",
			TEMPFILENAME, TEMPFILENAME))
		return 0;
	    ...
	    if (im->zsize == 4)
	      {
		if (!exec_cmd("/usr/sbin/oneband %s.rgb %s.r 0",
			      TEMPFILENAME, TEMPFILENAME))
		    return 0;
		if (!exec_cmd("/usr/sbin/oneband %s.rgb %s.g 1",
			      TEMPFILENAME, TEMPFILENAME))
		    return 0;
		if (!exec_cmd("/usr/sbin/oneband %s.rgb %s.b 2",
			      TEMPFILENAME, TEMPFILENAME))
		    return 0;
		if (!exec_cmd("/usr/sbin/oneband %s %s.a 3",
			      fn, TEMPFILENAME))
		    return 0;
		if (!exec_cmd("/usr/sbin/cglue %s.r %s.g %s.b %s.a %s.rgba",
			TEMPFILENAME, TEMPFILENAME,
		...
		if (!exec_cmd("/usr/sbin/oneband %s %s.a 3",
			      fn, TEMPFILENAME))
		    return 0;
		if (!exec_cmd("/usr/sbin/cglue %s.r %s.a %s.rgba",
			TEMPFILENAME, TEMPFILENAME, TEMPFILENAME))
		    return 0;
	...
	else
	  {
		...
		*(cto++) = *c;
		...
		readtex(name,&MipMapFile.tex[lvl],fmt,siz,makestatic,lr, lg, lb, hr, hg, hb,
			MIPMAP,
			(flags & ~MM_SUPPLIED_FLAG) | MM_HI_LEVEL,
			shuffle_mask);
	...
	if (output == C) printCheader(siz, im, fmt, tex);
	else if (output == MIPMAP) printMipMapheader(siz, im, fmt, tex);
	else printRAWheader(siz, im, fmt, tex);

	tex->width = im->xsize;
	tex->height = im->ysize;

	if (output == C) printpreview(im, buf);
	if (output == MIPMAP) printpreview(im, buf);
	...
	if (((output == C) || (output == MIPMAP)) && !(flags & SKIP_RAW_FLAG)) {      
	    if( flags & XTRA_FLAG )
	        printf(
	    "static Gfx %s%s_dummy_aligner1[] = { gsSPEndDisplayList(), gsSPEndDisplayList(),\n\tgsSPEndDisplayList(), gsSPEndDisplayList() };\n\n",
			tex->name, ((output == C)?"_C":"_MIP"));
	    else
	        printf(
	    "static Gfx %s%s_dummy_aligner1[] = { gsSPEndDisplayList() };\n\n",
			tex->name, ((output == C)?"_C":"_MIP"));
	...
	for (; y != (yfinal+dy); y+=dy) {
		...
		num_bytes += printdata(im, siz, fmt, i, red, green, blue, alpha, intensity, output, flags, cur_shuf_mask);
	...
	if ((output == C) && !(flags & SKIP_RAW_FLAG)) printf ("};\n\n");
	...
	if (fmt == CI)
	  {
	    ...
	    if (!exec_cmd("/sbin/rm -f %s %s.r %s.g %s.b %s.a %s.ppm %s.rgb %s.rgba %s.fooa %s.foob", 
