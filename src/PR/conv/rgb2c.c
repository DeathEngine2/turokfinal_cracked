...
"          -S smask   Swap 32-bit words on odd lines (smask = bit to toggle in texels)\n"
...
"          -b         toggle shifting each tile by 0.5.  This is neccessary\n"
...
int main (int argc, char *argv[])
{
			...
			case 'o':
				if (strcasecmp (optarg, "C") == 0) {
					...
					flags &= ~MM_SUPPLIED_FLAG;
				        ...
				        output = MIPMAP;
					flags &= ~MM_SUPPLIED_FLAG;
                                } else if (strcasecmp (optarg, "MIPSUPPLIED") 
									== 0){
				        output = MIPMAP;
					flags |= MM_SUPPLIED_FLAG;
					flags ^= HALF_SHIFT;
	...
	if (output != MIPMAP) flags &= ~SKIP_RAW_FLAG;
	...
	readtex (argv[optind], &tex, fmt, siz, 0, lr, lg, lb, hr, hg, hb, output, flags, shuf_mask);
}
