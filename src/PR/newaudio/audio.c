...
void initAudio(void) 
{
   ...
   ALADPCMWaveInfo *adpcmWave;
    ...
    romCopy(_seqbankSegmentRomStart, (char *)bankPtr, bankLen);
    alBnkfNew(bankPtr, (u8 *) _seqtableSegmentRomStart);
	...
	//SortSounds(bankPtr->bankArray[0]->instArray[0]);
		...
		SeqInfo[0].StartAddr = _seq1SegmentRomStart;

		SeqInfo[1].size = _seq2SegmentRomEnd - _seq2SegmentRomStart;
		SeqInfo[1].StartAddr = _seq2SegmentRomStart;

		SeqInfo[2].size = _seq3SegmentRomEnd - _seq3SegmentRomStart;
		SeqInfo[2].StartAddr = _seq3SegmentRomStart;

#define SELECTED_SEQ			2
#if 1
    // Load the sequence file from ROM
    seqLen = SeqInfo[SELECTED_SEQ].size;
    seqPtr = alHeapAlloc(&hp, 1, seqLen);
    romCopy(SeqInfo[SELECTED_SEQ].StartAddr, (char *) seqPtr, seqLen);
   ...
   alSeqNewMarker(seq, &seqStart, 0);
	//alSeqNewMarker(seq, &seqEnd, TICKS_PER_MEASURE*108);
   ...
   alSeqpSetBank(seqp, bankPtr->bankArray[0]);
...
#if 0
	wavetable = bankPtr->bankArray[0]->instArray[0]->soundArray[0]->wavetable;
	adpcmWave = &wavetable->waveInfo.adpcmWave;
	...
	{
		//adpcmWave->loop->end = 17000;
		//pcmWave->loop->end = adpcmWave->loop->start + (150*16);

		PRINTF("loop start: %d\n", adpcmWave->loop->start);
		PRINTF("loop end: %d\n", adpcmWave->loop->end);
