...
extern u8 _seq0SegmentRomStart[], _seq0SegmentRomEnd[];
...
extern u8 _seq4SegmentRomStart[], _seq4SegmentRomEnd[];
extern u8 _seq5SegmentRomStart[], _seq5SegmentRomEnd[];
extern u8 _seq6SegmentRomStart[], _seq6SegmentRomEnd[];
extern u8 _seq7SegmentRomStart[], _seq7SegmentRomEnd[];
extern u8 _seq8SegmentRomStart[], _seq8SegmentRomEnd[];
extern u8 _seq9SegmentRomStart[], _seq9SegmentRomEnd[];
...
void initAudio(void) 
{
   ...
   alHeapInit(&hp, audioHeap, AUDIO_HEAP_SIZE);
	...
	if(NumSeq == 0)
	{
		PRINTF("NUMBER OF SEQUENCES NOT SPECIFIED\n");
		return;
	}

	if(NumSeq > 10)
	{
		PRINTF("NUMBER OF SEQUENCES TOO BIG (%d) \n", NumSeq);
		return;
	}
	...
	for(i = 0; i < NumSeq; i++)
	{	
		SeqInfo[i].RAMaddr = alHeapAlloc(&hp, 1, SeqInfo[i].size);

		if(SeqInfo[i].RAMaddr)
		{
			romCopy(SeqInfo[i].ROMaddr, (char *) SeqInfo[i].RAMaddr, SeqInfo[i].size);
		}
		else
			PRINTF("MEMORY ALLOCATION ERROR: unable allocate seq %d\n", i+1);
	}
...
void SetupROMaddrs(void)
{
	...
	SeqInfo[0].size = _seq0SegmentRomEnd - _seq0SegmentRomStart;
	SeqInfo[0].ROMaddr = _seq0SegmentRomStart;

	SeqInfo[1].size = _seq1SegmentRomEnd - _seq1SegmentRomStart;
	SeqInfo[1].ROMaddr = _seq1SegmentRomStart;

	SeqInfo[2].size = _seq2SegmentRomEnd - _seq2SegmentRomStart;
	SeqInfo[2].ROMaddr = _seq2SegmentRomStart;

	SeqInfo[3].size = _seq3SegmentRomEnd - _seq3SegmentRomStart;
	SeqInfo[3].ROMaddr = _seq3SegmentRomStart;

	SeqInfo[4].size = _seq4SegmentRomEnd - _seq4SegmentRomStart;
	SeqInfo[4].ROMaddr = _seq4SegmentRomStart;

	SeqInfo[5].size = _seq5SegmentRomEnd - _seq5SegmentRomStart;
	SeqInfo[5].ROMaddr = _seq5SegmentRomStart;

	SeqInfo[6].size = _seq6SegmentRomEnd - _seq6SegmentRomStart;
	SeqInfo[6].ROMaddr = _seq6SegmentRomStart;

	SeqInfo[7].size = _seq7SegmentRomEnd - _seq7SegmentRomStart;
	SeqInfo[7].ROMaddr = _seq7SegmentRomStart;

	SeqInfo[8].size = _seq8SegmentRomEnd - _seq8SegmentRomStart;
	SeqInfo[8].ROMaddr = _seq8SegmentRomStart;

	SeqInfo[9].size = _seq9SegmentRomEnd - _seq9SegmentRomStart;
	SeqInfo[9].ROMaddr = _seq9SegmentRomStart;
