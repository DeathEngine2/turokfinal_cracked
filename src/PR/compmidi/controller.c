...
void UpdateController(void)
{
    ...
    u16         firstCntrl = 1;
	 int			handle;
	static float	newpitch = .0625;	
	static int		seqnum = 0;	
	static int		globalsfxnum = 1;
   ...
   for( i = 0; i < MAXCONTROLLERS; i++)
   {
		if((validcontrollers >> i) & 1) 
		{
			...
			if (firstCntrl) 
			{ /* joy stick from first controller only */
			...
			{  
				...
				SetupSeq(seqnum);
#endif
				PRINTF("Looped sound started\n");
				handle = initSFX(0, 100);
				SetSoundVolume(handle, .7);
				//SetSoundPitch(handle, .5);
				playSFX(handle);
