...
int		UseUCode            =  0;		/* 1=dram 2=fifo 0=dmem */
 ...
 */
u64 dram_stack[SP_DRAM_STACK_SIZE64+2];	/* used for matrix stack */
...
void CreateTaskStructure(int n)
{
  ...
  switch (UseUCode) {
    case 0: /* xbus */
      tlistp[n]->t.ucode      	    = (u64 *) gspFast3DTextStart;
      tlistp[n]->t.ucode_data 	    = (u64 *) gspFast3DDataStart;
      tlistp[n]->t.flags      	    = OS_TASK_DP_WAIT;
      break;
    case 1: /* dram */
      tlistp[n]->t.ucode      	    = (u64 *) gspFast3D_dramTextStart;
      tlistp[n]->t.ucode_data 	    = (u64 *) gspFast3D_dramDataStart;
      tlistp[n]->t.output_buff 	    = rdp_output;
      tlistp[n]->t.output_buff_size = (u64 *) &rdp_output_len;
      tlistp[n]->t.flags	    = OS_TASK_DP_WAIT;
      break;
    case 2: /* fifo */
      tlistp[n]->t.ucode      	    = (u64 *) gspFast3D_fifoTextStart;
      tlistp[n]->t.ucode_data 	    = (u64 *) gspFast3D_fifoDataStart;
      tlistp[n]->t.output_buff 	    = rdp_output;
      tlistp[n]->t.output_buff_size = (u64 *) ((int)rdp_output + (int)OutLen);
      tlistp[n]->t.flags	    = 0;
      break;
...
static void SendDisplayLists(void)
{
  ...
  OSTime startallsp, startalldp, endallsp, endalldp;
  int    i;

  dpTask = (UseUCode == 1) ? 0 : 1;
  spTask = 1;

  osSpTaskLoad(tlistp[spTask]);
  spstart = startallsp = dpstart = startalldp = osGetTime();
  ...
  while ((dpTask <= numtasks) || (spTask < numtasks)) {
	...
	switch (UseUCode) {
	  ...
	  case 1: /* dram */
	    osInvalDCache(&rdp_output_len, 8);
    	    if ((int)rdp_output_len>RDP_OUTPUT_BUF_LEN) {
      	      PRINTF("ERROR: rdp_output_len=%x > %x (%d > %d)\n",
			(int)rdp_output_len,
			(int)RDP_OUTPUT_BUF_LEN,
			(int)rdp_output_len,
			(int)RDP_OUTPUT_BUF_LEN);
      	      while(1) ;
    	    }
	    dpstart = osGetTime();
	    dpTask = spTask;
    	    osDpSetNextBuffer(rdp_output, rdp_output_len);
	    TimePerFrameSp[spTask] = 
		1000.0*(float)(spend - spstart) /((OSTime) (TicksPerSec));
	    break;
	  case 2: /* fifo */
	...
	switch (UseUCode) {
	  ...
	  case 1: /* dram */
		...
		dpTask = spTask;
	    TimePerFrameDp[spTask-1] = 
		1000.0*(float)(dpend - dpstart) /((OSTime) (TicksPerSec));
	    TimePerFrame[spTask-1] = 
		TimePerFrameDp[spTask-1] + TimePerFrameSp[spTask-1];
	    break;
	  case 2: /* fifo */
	    TimePerFrameDp[1] = 
		1000.0*(float)(dpend - dpstart) /((OSTime) (TicksPerSec));
...
static void CleanupAndSendDisplayList(void)
{
  ...
  if (DumpGbi) 
      guParseGbiDL((u64 *)tlistp[1]->t.data_ptr,
          (u32)tlistp[1]->t.data_size, GU_PARSEGBI_DUMPONLY);
...
static void PrintStatNumbers(int DisplayListLength)
{
  ...
  PRINTF(" Ucode: %s  ",(UseUCode) ? ((UseUCode==2) ? "fifo" : "dram") : "xbus");
  if (UseUCode==2)
      PRINTF(" BUFLEN=0x%x bytes",OutLen);
  PRINTF("\n");
...
static void PrintController3Instructions(void)
{
  ...
  PRINTF("B button     -> Do a GBI hex dump (for gbi2mem)      \n");
  PRINTF("G button     -> toggle controller mode (0,1,2,3)     \n");
  PRINTF("Arrow Up     -> increase DRAM FIFO buffer size       \n");
  PRINTF("Arrow Down   -> decrease DRAM FIFO buffer size       \n");
  PRINTF("Arrow Right  -> increase DRAM FIFO buffer size fast  \n");
  PRINTF("Arrow Left   -> decrease DRAM FIFO buffer size fast  \n");
...
static void doPoly(void *arg)
{
  ...
  while (1) 
	...
	for (i=1; i<=numtasks; i++) {
      ...
      if (ControllerInput && !Quiet) PrintStats(DisplayListLength);
...
static void main(void *arg)
{
  ...
  for (i=0; i<MAXTASKS; i++) {
    ...
    tlistp[i]->t.output_buff_size	= &rdp_output_len;
  }
  dynamicp = &dynamic;
  cfb_ptrs[0] = cfb_16_a; 
  cfb_ptrs[1] = cfb_16_b; 
