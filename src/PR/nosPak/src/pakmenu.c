...
#if	0
  if ((data[PORT_ID].button & START_BUTTON)) return 2; // Return if START 
#else
  if (!(data[PORT_ID].button & START_BUTTON)) return 2;// Return if not START
#endif

  //
  // Check if CONTROLLER PAK plugged
  // -------------------------------
  if (osPfsInit(siMessageQ, &pfs, PORT_ID)) return 0;
  
  osSyncPrintf("Controller Pack checking...done\n");    /* for debug */
...
#endif
//  osViSetSpecialFeatures(OS_VI_GAMMA_ON|OS_VI_GAMMA_DITHER_OFF|
//			   OS_VI_DITHER_FILTER_ON|OS_VI_DIVOT_OFF);
