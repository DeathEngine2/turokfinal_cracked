...
$41:
 ...
 # 163	  osContGetReadData(data);			// Get controller data
	...
	.loc	2 167
 # 164	#if	0
 # 165	  if ((data[PORT_ID].button & START_BUTTON)) return 2; // Return if START 
 # 166	#else
 # 167	  if (!(data[PORT_ID].button & START_BUTTON)) return 2;// Return if not START
...
$42:
 ...
 # 169	
 # 170	  //
 # 171	  // Check if CONTROLLER PAK plugged
 # 172	  // -------------------------------
 # 173	  if (osPfsInit(siMessageQ, &pfs, PORT_ID)) return 0;
	lw	$4, 288($sp)
	addu	$5, $sp, 128
	li	$6, PORT_ID
	.livereg	0x0E00000E,0x00000000
...
$52:
 ...
 # 235	  nWaitMesg(retrace);
	...
	.loc	2 239
 # 236	#if	SCREEN_LOW
 # 237	  osViSetMode(&osViModeTable[OS_VI_NTSC_LPN1]);
 # 238	#else
 # 239	  osViSetMode(&osViModeTable[OS_VI_NTSC_HPF1]);
	la	$4, osViModeTable+720
	.livereg	0x0800000E,0x00000000
	jal	osViSetMode
	.loc	2 243
 # 240	#endif
 # 241	//  osViSetSpecialFeatures(OS_VI_GAMMA_ON|OS_VI_GAMMA_DITHER_OFF|
 # 242	//			   OS_VI_DITHER_FILTER_ON|OS_VI_DIVOT_OFF);
 # 243	  osViSwapBuffer((u32 *)FBUF_TOP);
	li	$4, -2144337920
	.livereg	0x0800000E,0x00000000
	jal	osViSwapBuffer
	.loc	2 244
