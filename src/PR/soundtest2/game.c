...
#define OS_PRIORTY_NORMAL		10
#define OS_PRIORTY_HIGH 		11

extern INT16 SwitchCounter;
//char	padbytes[8];

// yield buffer
char yield_buffer[OS_YIELD_DATA_SIZE+15];
char *yield_bufferptr;
/*
...
OSTask          taskHeader =
{
	...
	//0
	OS_YIELD_DATA_SIZE	/* yield buffer length */
};
...
#define NUMMODES 4
	...
	if (dumpmenu[DM_RSPRAW].val ==1.0) {
		...
		guDumpGbiDL(theadp,0);
		for (i=0; i<100; i++) rmonPrintf("=====================================\n");
		rmonPrintf("\n\n==========DONE WITH DUMP==========\n\n");
	}
