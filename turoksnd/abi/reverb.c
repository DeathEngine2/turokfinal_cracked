...
s32 alFxParam(void *filter, s32 paramID, void *param)
{
	...
	/*
	 * the following code assumes that _FX_CTRL_x controllers
	 * are contiguous, sequential and increasing
	 */

      case AL_MIDI_FX_CTRL_0:
      case AL_MIDI_FX_CTRL_1:
      case AL_MIDI_FX_CTRL_2:
	n = (u8)paramID - AL_MIDI_FX_CTRL_0;
	if (param) {
	    /* r->delay[n].length = L_INC[n] * (s32)param; */
	}
	break;
	...
	n = (u8)paramID - AL_MIDI_FX_CTRL_3;
	/* r->delay[n].coef = ((s16)param) << 8; */
	break;
...
#ifdef _DEBUG
      if (length+(s32)d->rsdelta > r->length) {
